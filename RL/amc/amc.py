...
from env import ChannelPruningEnv
...
def get_model_and_checkpoint(model, dataset, checkpoint_path, n_gpu=1):
    if model == 'mobilenet' and dataset == 'imagenet':
        from models.imagenet.mobilenet import MobileNet
        net = MobileNet(n_class=1000)
    elif model == 'mobilenetv2' and dataset == 'imagenet':
        from models.imagenet.mobilenet_v2 import MobileNetV2
...
def train(num_episode, agent, env, output,args):
    ...
    while episode < num_episode:  # counting based on episode
        ...
        if done:  # end of episode
            print('#{}: episode_reward:{:.4f} {}: {:.4f},acc_another:{}, ratio: {:.4f}'.format(episode, episode_reward,args.acc_metric,
                                                                                 info['accuracy'],info['accuracy_'],
                                                                                 info['compress_ratio']))
            text_writer.write(
                '#{}: episode_reward:{:.4f} {}: {:.4f},acc_another:{}, ratio: {:.4f}\n'.format(episode, episode_reward,args.acc_metric,
                                                                                 info['accuracy'],info['accuracy_'],
                                                                                 info['compress_ratio']))
            final_reward = T[-1][0]
